...
function getMatchesLongShort(idSearch) {
  fetch(`https://api.opendota.com/api/players/${idSearch}/matches`)
    ...
    .then(function (matchesArray) {
      ...
      winOrLoss(matchesArray);
    });
}

function winOrLoss(matchesArray) {
  for (let i = 0; i < matchesArray.length; i++) {
    ...
    if (arrayDuration < startingSDuration) {
      ...
      deathsS = deaths;
      assistsS = assists;
      shortest = Math.round(startingSDuration / 60);
      // console.log("*** INSIDE CONDITION 2 ****");
      // console.log(playerSlot, radiantWin, shortestWin, shortMatchId);
    }

    console.log("**** LONGEST AFTER CON 2 *****");
    console.log(arrayDuration);
    console.log(longest);
    fetchHeroInformation(longestWin, shortestWin, longest, shortest);
  }
}

// function genericFun(duration, ){

// }

// Fetch hero information for each hero ID
function fetchHeroInformation(longestWin, shortestWin, longest, shortest) {
  const heroInfoUrl =
